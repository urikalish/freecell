...
src/
  ...
  styles/
    ...
    difficulty.css        # Difficulty-selection overlay
...
- After every player move (`tryMove`) and after `newGame`, `findSafeFoundationMoves` is called to automatically advance unambiguous cards to foundations. In `newGame` this goes through the `autoMoveToFoundations` helper; in `tryMove` it is called inline to also capture animation rects before re-render.
