...
src/
  ...
  model/
    ...
    moves.ts            # Move validation, execution, undo, getMovableCards, hasAnyValidMoves
  data/
  ...
  styles/
    ...
    no-moves.css        # "No valid moves" banner
...
`renderer.ts` exports pure render functions. `main.ts` calls `app.innerHTML = renderGame(state, selectedCardId, validTargets, noValidMoves)` on every state change — a full synchronous re-render. There is no virtual DOM or diffing. The optional `noValidMoves` flag causes a "No valid moves" banner to be rendered centred over the playing area. Two additional exports render overlays on demand: `renderVictoryOverlay()`, and `renderDifficultyOverlay(currentDifficulty)` (renders the New Game dialog with a difficulty `<select>` and a deal number `<input>`).
...
- After every re-render, `hasAnyValidMoves` is evaluated (scanning all occupied free cells and the movable sequence top of each tableau column) to set the `noValidMoves` flag. The banner is suppressed before the first move, while animating, and once the game is won.
